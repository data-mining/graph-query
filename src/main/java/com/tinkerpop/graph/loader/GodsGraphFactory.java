...
/**
 * @author kaniska
 * Indexing turned-off for the sake of simplicity
 * Prereq. for Indexing - start graph with ES configuration = TitanFactory.open('conf/titan-cassandra-es.properties')
 */
public class GodsGraphFactory {
	...
	public static TitanGraph create(final String directory) {
		...
		// config.set("index." + INDEX_NAME + ".directory", directory +
		// File.separator + "es");
		// config.set("index." + INDEX_NAME + ".elasticsearch.local-mode",
		// true);
		// config.set("index." + INDEX_NAME + ".elasticsearch.client-only",
		// false);
	...
	public static void load(final TitanGraph graph, String mixedIndexName, boolean uniqueNameCompositeIndex) {
		...
		PropertyKey datetime = mgmt.makePropertyKey("datetime").dataType(Date.class).make();

		PropertyKey reason = reason = mgmt.makePropertyKey("reason").dataType(String.class).make();
		...
		Vertex minset = tx.addVertex(T.label, "human", "name", "minset");
		
		Vertex robin = tx.addVertex(T.label, "monster", "name", "robin");
		...
		hercules.addEdge("battled", nemean, "time", 1, "place", Geoshape.point(38.1f, 23.7f) , "datetime", new Date());
		hercules.addEdge("battled", hydra, "time", 2, "place", Geoshape.point(37.7f, 23.9f), "datetime", new Date());
		hercules.addEdge("battled", cerberus, "time", 12, "place", Geoshape.point(39f, 22f), "datetime", new Date());
		hercules.addEdge("battled", minset, "time", 20, "place", Geoshape.point(49f, 32f));
		hercules.addEdge("battled", robin, "time", 25, "place", Geoshape.point(59f, 42f));
